...
FrameCollectorDelegate, HTTPHandlerDelegate {
    ...
    public init(doLog: @escaping ((String)->Void),
                transport: Transport,
    ...
    public func stop(closeCode: UInt16 = CloseCode.normal.rawValue) {
        ...
        write(data: payload, opcode: .connectionClose, completion: { [weak self] in
            ...
            self?.reset()
            self?.doLog("WSEngine.stop writeCompletion afterReset")
            self?.forceStop()
    ...
    public func write(data: Data, opcode: FrameOpCode, completion: (() -> ())?) {
        ...
        writeQueue.async { [weak self] in
            ...
            guard let s = self else {
                self?.doLog("WSEngine.write cb exit noSelf")
                return
            }
            self?.doLog("WSEngine.write cb self")
            s.mutex.wait()
            let canWrite = s.canSend
            s.mutex.signal()
            if !canWrite {
                self?.doLog("WSEngine.write cb exit cannotWrite")
                return
            }
            self?.doLog("WSEngine.write cb canWrite")
            ...
            self?.doLog("WSEngine.write cb creatingFrame")
            let frameData = s.framer.createWriteFrame(opcode: opcode, payload: sendData, isCompressed: isCompressed)
            self?.doLog("WSEngine.write cb writing")
            s.transport.write(data: frameData, completion: {_ in
                self?.doLog("WSEngine.write cb written entry")
                completion?()
                self?.doLog("WSEngine.write cb written exit")
            })
